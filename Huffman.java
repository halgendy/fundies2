...
class Huffman {
  ...
  public ArrayList<Boolean> encode(String input) {
    ...
    for (char ch : charArr) {
      ...
      binaryCode = this.root.getCode(ch, "");
  ...
  public String decode(ArrayList<Boolean> encoded) {
    String ans = "";
    TotalNode currentNode = this.root;
    ...
    for (int i = 0; i < encoded.size(); i++) {
      ...
      if (currentNode != null && currentNode.letter == null && i == encoded.size() - 1
          && !currentNode.equals(this.root)) {
        ans += '?';
      }
    }
    return ans;
...
interface INode {
  ...
  Integer combineFreqLeaf(LeafNode thatLeaf);
  
  Integer combineFreqTotal(TotalNode thatLeaf);
  
  String getCode(char ch, String ans);
  
  Boolean greaterThan(INode that);
  
  Boolean greaterThanLeaf(LeafNode that);
  
  Boolean greaterThanTotal(TotalNode that);
}
...
class LeafNode implements INode {
  ...
  public Integer combineFreq(INode that) {
    return that.combineFreqLeaf(this);
  }
    
  public Integer combineFreqLeaf(LeafNode thatLeaf) {
    return this.freq + thatLeaf.freq;
  }
  
  public Integer combineFreqTotal(TotalNode thatTotal) {
    return this.freq + thatTotal.left.combineFreq(thatTotal.right);
  }
  
  // a depth first search for char c
  public String getCode(char ch, String ans) {
  ...
  public Boolean greaterThan(INode that) {
    return that.greaterThanLeaf(this);
  }
  
  public Boolean greaterThanLeaf(LeafNode that) {
    return this.freq < that.freq;
  }
  
  public Boolean greaterThanTotal(TotalNode that) {
    return this.freq < that.freq;
  }
}
...
class TotalNode implements INode {
  ...
  public Integer combineFreq(INode that) {
    return that.combineFreqTotal(this);
  }
  ...
  public String getCode(char ch, String ans) {
    ...
    return this.left.getCode(ch, ans + '0') + this.right.getCode(ch, ans + '1');
  }
  
  public Boolean greaterThan(INode that) {
    return that.greaterThanTotal(this);
  }
...
class ExamplesHuffman {
  ...
  ArrayList<INode> nodeArr1 = new ArrayList<INode>(Arrays.asList(new LeafNode("a", 12), new LeafNode("b", 45),
      new LeafNode("c", 5), new LeafNode("d", 13), new LeafNode("e", 9), new LeafNode("f", 16)));
  ArrayList<INode> nodeArr2 = new ArrayList<INode>(Arrays.asList(new LeafNode("a", 45), new LeafNode("b", 45),
  ...
  void testBuildTree(Tester t) {
    ...
    TotalNode repH1 = new TotalNode(new LeafNode("b", 45),
        new TotalNode(new TotalNode(new LeafNode("a", 12), new LeafNode("d", 13)),
            new TotalNode(new TotalNode(new LeafNode("c", 5), new LeafNode("e", 9)), new LeafNode("f", 16))));
    
    TotalNode repH2 = new TotalNode(new LeafNode("b", 45),
        new TotalNode(new TotalNode(new TotalNode(new LeafNode("c", 5), new LeafNode("e", 9)),
            new TotalNode(new LeafNode("f", 9), new LeafNode("e", 13))), new LeafNode("a", 45)));
  ...
  void testISort(Tester t) {
    ...
    t.checkExpect(this.nodeArr1,
        new ArrayList<LeafNode>(Arrays.asList(new LeafNode("a", 12), new LeafNode("b", 45), new LeafNode("c", 5),
            new LeafNode("d", 13), new LeafNode("e", 9), new LeafNode("f", 16))));
    t.checkExpect(this.nodeArr2,
        new ArrayList<LeafNode>(Arrays.asList(new LeafNode("a", 45), new LeafNode("b", 45), new LeafNode("c", 5),
    ...
    t.checkExpect(this.nodeArr1,
        new ArrayList<LeafNode>(Arrays.asList(new LeafNode("c", 5), new LeafNode("e", 9), new LeafNode("a", 12),
    ...
    t.checkExpect(this.nodeArr2,
        new ArrayList<LeafNode>(Arrays.asList(new LeafNode("c", 5), new LeafNode("e", 9), new LeafNode("f", 9),
  ...
  void testEncode(Tester t) {
    ...
    t.checkException(
        new IllegalArgumentException(
            "Tried to " + "encode z but that is not part of the language."),
        this.h1, "encode", "abz");
